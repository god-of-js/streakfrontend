<template>
    ...
    <form @submit.prevent="uploadVideo" class="admin_form col-10 mx-auto">
        ...
        <div class="d-flex flex-column col"> 
            ...
            <textarea type="text" v-model="description" required class="ml-2 mr-2"/>
        </div>
        <div class="d-flex justify-content-between">
          <div class="upload_box d-flex align-items-center flex-column justify-content-center col-5 m-4"  @click="clickFile('file')" @dragover.prevent @drop.prevent="droppedFile($event,'video')"> 
           <i class="mdi mdi-upload"> </i>
            <div class="font_sm" video-name>Click to add Movie or Drag and drop Movie </div>
            <input type="file" class="file hide" @change="videofile"/>
          </div>
          <div class="upload_box d-flex align-items-center flex-column justify-content-center col-5 m-4" @click="clickFile('imgfile')" @dragover.prevent @drop.prevent="droppedFile($event,'img')"> 
           <i class="mdi mdi-upload"> </i>
            <div class="font_sm" img-name>Click to add image or Drag and drop Image </div>
            <input type="file" class="imgfile hide" @change="imgfile" />
          </div>
        </div>
        <div class="d-flex">
          <div class="d-flex flex-column m-2 col"> 
            <div class="text-left pl-2  font_sm">Movie Cast (seperate by commas)    </div>
            <input type="text" v-model="cast" required/>
          </div>
          <div class="d-flex flex-column m-2 col"> 
            <div class="text-left pl-2  font_sm">Type    </div>
             <select v-model="movieType" required>
                <option v-for="(cat, index) in type" :key="index">
                {{cat.name}}
                </option>
                </select>
          </div>
        ...
        <div class="d-flex">
          <div class="d-flex flex-column m-2 col"> 
                ...
                <select v-model="categorySelect" @change="addCategory" required>
                   <option v-for="(cat, index) in moviecat" :key="index" @change="addCategory(cat.name)">
                        {{cat.name}}
                   </option>
                </select>
                <small v-if="category.length !== 0" class="font_sm"> <span v-for="(cat, index) in category" :key="index"> {{cat}}, </span></small>
          </div>
          <div class="d-flex flex-column m-2 col"> 
       ...
       <div class="d-flex justify-content-end pr-4"><button class="btn">Upload Video</button> </div> 
             </form>
    </div>
</template>
<script>
