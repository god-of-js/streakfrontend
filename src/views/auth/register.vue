<template>
    <div class="">
        <form class="auth_form d-flex flex-column pt-4 pb-5" @submit.prevent="register">
    ...
    <input type="password" class="m-2 p-2 " required v-model="data.password"
    ...
    <input type="password" class=" m-2 p-2" required  v-model="cpassword"
            oninvalid="this.setCustomValidity('Min. 8 characters with at least one capital letter, a number and a special character.')"
            oninput="this.setCustomValidity('')"
            min="8"
            pattern="(?=.*\d)(?=.*[a-z])(?=.*[!@#\$%\^&\*])(?=.*[A-Z]).{8,}"/>
        </div>
    <button class="btn blue_btn m-2 mt-4 d-flex justify-content-center align-items-center ">Sign Up </button>
     </form>
     <div class="font_sm">Part of BlueStreak? <router-link to="/login">Sign In </router-link> </div>
    </div>
</template>
<script>
